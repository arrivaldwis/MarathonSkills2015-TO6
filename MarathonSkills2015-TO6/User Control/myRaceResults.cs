...
namespace MarathonSkills2015_TO6.User_Control
{
    public partial class myRaceResults : UserControl
    {
        ...
        {
            var getRegId = data.Registrations.Where(x => x.RunnerId.Equals(this.runnerId)).Select(x => x.RegistrationId).FirstOrDefault();
            if (getRegId != null)
            {
            ...
            {
                MessageBox.Show("This runner don't have race result");
            }
        }
